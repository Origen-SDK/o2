...
pub fn parse_file(path: &Path, strict: bool) -> Result<Node<STIL>> {
    if path.exists() {
...
pub fn parse_str(stil: &str, source_file: Option<&str>, strict: bool) -> Result<Node<STIL>> {
    match STILParser::parse(Rule::stil_source, stil) {
        Err(e) => Err(Error::new(&format!("{}", e))),
        Ok(mut stil) => {
            let p = ParseRunner { strict };
            Ok(p.to_ast(stil.next().unwrap(), source_file)?.unwrap())
        }
    }
}
...
impl ParseRunner {
    fn build_expression(&self, pair: Pair<Rule>) -> Result<Node<STIL>> {
        let mut pairs = pair.into_inner();
        let p2 = pairs.next().unwrap();
        let mut term = self.to_ast(p2, None)?.unwrap();
        let mut done = false;
        while !done {
            if let Some(next) = pairs.peek() {
                match next.as_rule() {
                    Rule::add => {
                        pairs.next();
                        let next_term = self.to_ast(pairs.next().unwrap(), None)?.unwrap();
    ...
    fn build_term(&self, pair: Pair<Rule>) -> Result<Node<STIL>> {
        let mut pairs = pair.into_inner();
        let mut term = self.to_ast(pairs.next().unwrap(), None)?.unwrap();
        let mut done = false;
        while !done {
            if let Some(next) = pairs.peek() {
                match next.as_rule() {
    ...
    pub fn to_ast(&self, mut pair: Pair<Rule>, source_file: Option<&str>) -> Result<AST<STIL>> {
        let mut ast = AST::new();
        ...
        loop {
            ...
            match pair.as_rule() {
                ...
                Rule::expression | Rule::expression_ => {
                    ast.push(self.build_expression(pair)?);
                }
                Rule::time_expr => {
