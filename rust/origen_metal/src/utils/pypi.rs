...
pub fn get_package_details_from_test_server<S: AsRef<str>>(
    package_name: S,
) -> Result<serde_json::Value> {
    get_package_details_from(package_name, *PYPI_TEST_URL_BASE)
}

pub fn get_package_details_from<S1: AsRef<str>, S2: AsRef<str>>(
    package_name: S1,
    url: S2,
) -> Result<serde_json::Value> {
...
pub fn get_package_versions_from_test_server<S: AsRef<str>>(
    package_name: S,
) -> Result<Vec<String>> {
    get_package_versions_from(package_name, *PYPI_TEST_URL_BASE)
}

pub fn get_package_versions_from<S1: AsRef<str>, S2: AsRef<str>>(
    package_name: S1,
    url: S2,
) -> Result<Vec<String>> {
    let res = get_package_details_from(package_name.as_ref(), url)?;
    let versions = res["releases"]
        .as_object()
        .unwrap()
        .keys()
        .map(|k| k.to_string())
...
pub fn is_package_version_available<S1: AsRef<str>, S2: AsRef<str>>(
    package_name: S1,
    version: S2,
) -> Result<bool> {
...
pub fn is_package_version_available_on_test_server<S1: AsRef<str>, S2: AsRef<str>>(
    package_name: S1,
    version: S2,
) -> Result<bool> {
