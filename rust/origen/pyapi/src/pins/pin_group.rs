...
impl PinGroup {
    ...
    fn get_id(&self) -> PyResult<String> {
        ...
        match grp {
            Some(_grp) => Ok(_grp.id.clone()),
            Option::None => Err(PyErr::from(Error::new(&format!(
                "Stale reference to pin group {}",
                self.id
            )))),
        }
    }
    ...
    fn get_little_endian(&self) -> PyResult<bool> {
        ...
        match grp {
            Some(_grp) => Ok(_grp.is_little_endian()),
            Option::None => Err(PyErr::from(Error::new(&format!(
                "Stale reference to pin group {}",
                self.id
            )))),
