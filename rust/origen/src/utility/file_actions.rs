...
pub fn insert_before(file: &Path, regex: &Regex, text: &str) -> Result<bool> {
    modify(file, regex, Some(text), Operation::InsertBefore)
}
...
pub fn insert_before_all(file: &Path, regex: &Regex, text: &str) -> Result<bool> {
    modify(file, regex, Some(text), Operation::InsertBeforeAll)
}
...
pub fn insert_after(file: &Path, regex: &Regex, text: &str) -> Result<bool> {
    modify(file, regex, Some(text), Operation::InsertAfter)
}
...
pub fn remove_line(file: &Path, regex: &Regex) -> Result<bool> {
    modify(file, regex, None, Operation::RemoveLine)
}
...
pub fn remove_line_all(file: &Path, regex: &Regex) -> Result<bool> {
    modify(file, regex, None, Operation::RemoveLineAll)
}
...
pub fn replace(file: &Path, regex: &Regex, text: &str) -> Result<bool> {
    modify(file, regex, Some(text), Operation::Replace)
}
...
pub fn replace_all(file: &Path, regex: &Regex, text: &str) -> Result<bool> {
    modify(file, regex, Some(text), Operation::ReplaceAll)
}

fn modify(file: &Path, regex: &Regex, text: Option<&str>, operation: Operation) -> Result<bool> {
        ...
        if modified && !do_all {
            ...
            if regex.is_match(line) {
                ...
                match operation {
                    ...
                    Operation::Replace => {
                        output += &regex.replace(line, text.unwrap());
                    }
                    Operation::ReplaceAll => {
                        output += &regex.replace_all(line, text.unwrap());
                    }
                }
                modified = true;
            } else {
