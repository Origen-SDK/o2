...
use crate::{producer, app_config, STATUS};
...
impl FlattenText {
  ...
  fn to_text(&self, content: &str) -> Node {
    ...
    node!(Text, format!("{}{}", " ".to_string().repeat(spacing_length), content))
  }
  ...
  fn section_boundary(&self) -> Node {
    ...
    node!(Text, format!("{}{}", " ".to_string().repeat(spacing_length), self.boundary_string.repeat(boundary_repeat)))
  }
}
