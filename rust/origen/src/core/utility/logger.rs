...
impl Logger {
  ...
  fn _log(&self, prefix: &str, message: &str, ref_to_print_func: &dyn Fn(&str)) {
    self._out(&format!("{}{}", self._prefix(prefix), message), ref_to_print_func);
  }
  ...
  fn _log_block(&self, prefix: &str, messages: &Vec<&str>, ref_to_print_func: &dyn Fn(&str)) {
    if messages.len() == 0 {
      ...
      self._out(&format!("{}{}", self._prefix(prefix), messages[0]), ref_to_print_func);
    } else {
  ...
  fn _write_header(&self) {
    ...
    out +=             &format!("### Exec:    {}\n", env::current_exe().expect("Could not get the current EXE path!").to_string_lossy());
      ...
      file_handler: match fs::File::create(f) {
        ...
        Err(_e) => match fs::create_dir(f.parent().unwrap()) {
          Ok(_d) => match fs::File::create(f) {
            Ok(f) => f,
            Err(_e) => panic!("Could not open log file at {}", format!("{}", f.to_string_lossy()))
          },
          Err(_e) => panic!("Could not open log file at {}", format!("{}", f.to_string_lossy()))
        }
      },
