...
pub(crate) fn run(invocation: &ArgMatches, mut app: &App, exts: &Extensions, plugins: Option<&Plugins>, app_cmds: &AppCmds) -> origen::Result<()> {
    ...
    match subcmd.0 {
        ...
        // }
        "commands" => {
            if sub.subcommand().is_some() { // invocation.subcommand() {
