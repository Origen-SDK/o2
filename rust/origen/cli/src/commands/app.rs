...
pub(crate) fn add_helps(helps: &mut CmdHelps, app_cmds: &AppCmds) {
    helps
        ...
        .set_help_msg("Manage and interface with the application")
...
pub(crate) fn add_commands<'a>(
    ...
    let mut app_subc = helps.core_cmd(BASE_CMD).arg_required_else_help(true);
    // .subcommand(
    //     Command::new("init")
...
pub(crate) fn run(
    ...
    let subcmd = invocation.subcommand().unwrap();
    let sub = subcmd.1;
    match subcmd.0 {
        // "init" => {
        ...
        "commands" => {
            if sub.subcommand().is_some() {
                ...
                launch_as(
                    ...
                    matches,
                    app,
                    exts.get_app_ext(&path),
                    plugins,
                    Some({
                        overrides.insert(
                            "dispatch_root".to_string(),
                            Some(format!("r'{}'", app_cmds.cmds_root()?.display())),
                        );
                        overrides
                    }),
                );
