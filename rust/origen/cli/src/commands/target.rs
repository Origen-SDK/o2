...
macro_rules! tnames {
    ($cmd:expr) => {
        $cmd.get_many::<String>(TARGETS_OPT)
            .unwrap()
            .map(|t| t.as_str())
            .collect::<Vec<&str>>()
    };
...
gen_core_cmd_funcs__no_exts__no_app_opts!(
    ...
    { |cmd: App| { cmd.arg(full_paths_opt!()).visible_alias("t") } },
    core_subcmd__no_exts__no_app_opts!("add", "Activates the given target(s)", {
        |cmd: App| {
            cmd.visible_alias("a")
    ...
    core_subcmd__no_exts__no_app_opts!("clear", "Deactivates any and all current targets", {
        |cmd: App| cmd.visible_alias("c")
    }),
    core_subcmd__no_exts__no_app_opts!("remove", "Deactivates the given target(s)", {
        |cmd: App| {
            cmd.visible_alias("r")
    ...
    core_subcmd__no_exts__no_app_opts!(
        "set",
        "Activates the given target(s) while deactivating all others",
        {
            |cmd: App| {
                cmd.visible_alias("s")
    ...
    core_subcmd__no_exts__no_app_opts!(
        "default",
        "Activates the default target(s) while deactivating all others",
        { |cmd: App| { cmd.visible_alias("d").arg(full_paths_opt!()) } }
    ),
    core_subcmd__no_exts__no_app_opts!("view", "Views the currently activated target(s)", {
        |cmd: App| cmd.visible_alias("v").arg(full_paths_opt!())
    })
);
...
fn view_targets(fp: bool) -> Result<()> {
    if let Some(targets) = target::get(fp) {
        ...
        println!(
            "No targets have been enabled and this workspace does not enable any default targets"
        )
    }
    Ok(())
}
