use super::_prelude::*;
use super::launch_as;
use crate::framework::aux_cmds::{add_aux_ns_helps, aux_ns_subcmd};
use crate::framework::{build_path, AuxCmds};
...
pub(crate) fn add_helps(helps: &mut CmdHelps, aux_cmds: &AuxCmds) {
    helps
        .add_core_cmd(BASE_CMD)
        .set_help_msg("Interface with auxillary commands")
...
pub(crate) fn run(
    ...
    aux_cmds: &crate::AuxCmds,
) -> origen::Result<()> {
    if let Some(subc) = cmd.subcommand() {
        let ns_ins = aux_cmds.namespaces.get(subc.0).expect(&format!(
            "Expected auxillary command namespace '{}' to be present, but was not found",
            subc.0
        ));
        ...
        launch_as(
            ...
            matches,
            app,
            exts.get_aux_ext(subc.0, &path),
            plugins,
            Some({
                overrides.insert(
                    "dispatch_root".to_string(),
                    Some(format!("r'{}'", ns_ins.root().display())),
                );
