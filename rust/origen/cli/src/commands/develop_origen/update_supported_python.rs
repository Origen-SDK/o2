...
pub(crate) fn update_supported_python_cmd<'a>() -> SubCmd<'a> {
    core_subcmd__no_exts__no_app_opts!(
        ...
        {
            |cmd| {
                cmd.arg(Arg::new("min_version").action(SetArg).required(true))
                    .arg(
                        Arg::new("non_inclusive_max_version")
                            .action(SetArg)
                            .required(true),
                    )
            }
...
pub(crate) fn run(invocation: &clap::ArgMatches) -> Result<()> {
    ...
    let max = invocation
        .get_one::<String>("non_inclusive_max_version")
        .unwrap();
    let version_str = format!("requires-python = \">={min},<{max}\"");
    ...
    let p = &STATUS
        .origen_wksp_root
        .join("rust/origen/cli/src/_generated/python.rs");
    println!("Creating python rs file {}", p.display());
    let mut f = File::create(p)?;
    f.write_fmt(format_args!(
        "// THIS IS AN AUTO GENERATED FILE FROM CMD {}\n",
        BASE_CMD
    ))?;
    f.write_fmt(format_args!(
        "pub const MIN_PYTHON_VERSION: &str = \"{}\";\n",
        min
    ))?;
    ...
    // TODO O2DevScripts Regenerate UV locks for test projects after updating supported Python versions.
...
fn update_pyproject(path: &Path, version_str: &str) -> Result<()> {
    ...
    let i = lines
        .iter()
        .position(|l| l.starts_with("requires-python = "))
        .unwrap();
    lines[i] = version_str;
