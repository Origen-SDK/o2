...
use super::{Applies, CmdOptCache, CmdSrc, Opt, OptTOML};
...
impl Extensions {
    ...
    where
        F: Fn(ExtensionTOML) -> Result<Option<Extension>>,
    {
    ...
    pub fn add_from_app_toml(&mut self, ext_toml: ExtensionTOML) -> Result<bool> {
        self.add_ext(ext_toml, |t| {
            Extension::from_extension_toml(ExtensionSource::App, t)
        })
    }

    pub fn add_from_pl_toml(&mut self, pl: &Plugin, ext_toml: ExtensionTOML) -> Result<bool> {
        self.add_ext(ext_toml, |t| {
            Extension::from_extension_toml(ExtensionSource::Plugin(pl.name.to_owned()), t)
        })
    }

    pub fn add_from_aux_toml(
        &mut self,
        ns: &AuxCmdNamespace,
        ext_toml: ExtensionTOML,
    ) -> Result<bool> {
        self.add_ext(ext_toml, |t| {
            Extension::from_extension_toml(ExtensionSource::Aux(ns.namespace(), ns.root()), t)
        })
    }
    ...
    pub fn get_pl_ext(&self, pl: &str, cmd_path: &str) -> Option<&Vec<Extension>> {
        self.extensions
            .get(&CmdSrc::Plugin(pl.to_string(), cmd_path.to_string()))
    }

    pub fn get_aux_ext(&self, ns: &str, cmd_path: &str) -> Option<&Vec<Extension>> {
        self.extensions
            .get(&CmdSrc::Aux(ns.to_string(), cmd_path.to_string()))
    }
}
...
impl Extension {
    pub fn from_extension_toml(
        ext_source: ExtensionSource,
        ext: ExtensionTOML,
    ) -> Result<Option<Self>> {
        let mut slf = Self {
        ...
        slf.opts = from_toml_opts!(
            ext.opt,
            &slf.extends.to_string(),
            Some(&slf.source.to_string())
        );
...
impl Applies for Extension {
    ...
    fn on_env_error_msg(&self, e: &String) -> String {
        format!(
            "Failed to parse 'on_env' '{}', extending '{}', for {}",
            e, self.extends, self.source
        )
    }
}
