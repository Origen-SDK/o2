...
def test_duplicate_instance_error():
    ...
    assert str(
        err_info.value
    ) == "Cannot create instance of 'example.blocks.dut.controller.Controller' named 'dut', it already exists!"
    # Using a class alias in the error message
    with pytest.raises(DuplicateInstanceError) as err_info:
        raise DuplicateInstanceError(dut, 'DUT')
    assert str(
        err_info.value
    ) == "Cannot create instance of 'DUT' named 'dut', it already exists!"
