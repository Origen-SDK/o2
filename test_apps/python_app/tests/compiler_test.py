...
class FixtureCompilerTest(abc.ABC):
    ...
    def dummy_input_filename(self):
        return pathlib.Path(str(self.expected_output_filename) + f'.{self.extension}')
    ...
    def expected_default_output_filename(self):
        ...
        return origen.app.output_dir.joinpath(f'renders/{s.__name__.lower()}/test_file.txt')

    @property
    def input_filename(self):
        return origen.root.joinpath('example/templates/dut_info.txt' + f'.{self.extension}')
    ...
    def test_compiler_resolves_default_filenames(self):
        # Test as string
        assert user_compiler().resolve_filename(str(self.dummy_input_filename)) == self.expected_default_output_filename

        # Test as pathlib.Path
        assert user_compiler().resolve_filename(self.dummy_input_filename) == self.expected_default_output_filename

    def test_compiler_resolves_filenames(self):
        # Test as string
        assert user_compiler().resolve_filename(str(self.dummy_input_filename), output_dir=tmp_dir()) == self.expected_output_filename

        # Test as pathlib.Path
        assert user_compiler().resolve_filename(self.dummy_input_filename, output_dir=tmp_dir()) == self.expected_output_filename
...
class TestCompilerStack():
    ...
    def test_compiler_renders_text(self, clean_falcon, clean_compiler):
        origen.app.compile(
            self.test_cases.str_render,
            direct_src=True,
            syntax='mako'
        )
        assert len(user_compiler().renders) == 1
        assert len(user_compiler().stack) == 0
        assert user_compiler().renders[0] == self.test_cases.expected_str_render

        origen.app.compile(
            self.test_cases.str_render_with_additional_context,
        ...
        assert len(user_compiler().stack) == 0
        assert user_compiler().renders[1] == self.test_cases.expected_str_render_with_additional_context
        assert user_compiler().renders[-1] == user_compiler().last_render

    def test_compiler_text_render_requires_syntax(self, clean_falcon, clean_compiler):
        assert len(user_compiler().stack) == 0
        with pytest.raises(origen.compiler.ExplicitSyntaxRequiredError):
            origen.app.compile(self.test_cases.str_render, direct_src=True)

    def test_compiler_returns_templates_dir(self):
        assert user_compiler().templates_dir == origen.app.root.joinpath('example/templates')
