...
class TestDefiningPins:
  ...
  def test_adding_pins_with_more_involved_reset_actions(self, clean_falcon):
    grp = origen.dut.add_pin("porta", width=4, reset_data=0xC, reset_action=PinActions("|A|CC|B|"))
    assert grp.data == 0xC
  ...
  def test_exception_on_invalid_reset_actions(self, clean_falcon):
    ...
    with pytest.raises(OSError):
      origen.dut.add_pin("invalid", width=2, reset_action=PinActions("Z"))
    with pytest.raises(OSError):
...
class TestSettingStates:
  ...
  def test_setting_pins_using_pin_actions_class(self, clean_falcon, pins, grp):
    grp = origen.dut.pin("grp")
