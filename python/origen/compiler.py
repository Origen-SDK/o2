                ...
                if isinstance(arg, pathlib.Path):
                    ...
                    self.__write_output_file(
                        curr_template.render(
                            dut=origen.standard_context()['dut'],
                            tester=origen.standard_context()['tester'],
                            origen=origen.standard_context()['origen']), arg)
                else:
                    ...
                    if self.syntax.inspect(arg):
                        ...
                        self.renders.append(
                            curr_template.render(
                                dut=origen.standard_context()['dut'],
                                tester=origen.standard_context()['tester'],
                                origen=origen.standard_context()['origen'],
                                **options))
                    else:
    ...
    def __check_args(self, *args):
        ...
        for arg in args:
            if not isinstance(arg, (str, pathlib.Path)):
                raise TypeError(
                    'Compiler arguments must be of type str or pathlib.Path')
            if isinstance(arg, pathlib.Path):
                if not arg.suffix == '.mako':
                    raise FileExtensionError('.mako')

    def __make_pathlibs_if_necessary(self, opts):
        for k, v in dict(filter(lambda k: '_dir' in k[0],
                                opts.items())).items():
