...
def check_consistency(app, env):
  ...
  for target in _unchecked_targets:
    ...
    if t.is_ref:
      ...
      if not any(p.match(f"{t.target}*") for p in list(pathlib.Path(app.srcdir).joinpath(t.target).parent.glob('*'))):
...
def href_to(target, *, docname=None):
  ...
  t = shorthand.get(target)
  if t is None:
    shorthand.logger.error(f"Unknown shorthand target from {app.env.docname} - '{target}'")
  else:
    if t.is_ref:
      ...
      if ':' in t.target:
        ...
        loc = _make_id(loc)
        doc = app.builder.get_relative_uri(docname or app.env.docname, doc)
        return f"{doc}#{loc}"
      else:
        return app.builder.get_relative_uri(docname or app.env.docname, t.target)
    elif t.is_doc or t.is_static:
      ...
      shorthand.logger.error("Cannot generate HREF for a substitution type!")
    else:
      shorthand.logger.error(f"Unknown category '{t.category}' for target '{t.name}'")
  return '#'
...
def templating_context(app):
  return {
    'shorthand': sys.modules[__name__],
    'link_to': lambda target, txt=None, **opts: link_to(target, txt, **opts),
    'href_to': lambda target, **opts: href_to(target, **opts),
    'anchor_to': lambda target, txt=None, **opts: anchor_to(target, txt, **opts),
  }

def _link_to(typ: str, rawtext: str, text: str, lineno: int, inliner: Inliner, options: Dict = {}, content: List[str] = []) -> Tuple[List[Node], List[system_message]]:
  '''
  ...
  if t is None:
    ...
    shorthand.logger.error(f"Unknown shorthand target from {app.env.docname}:{lineno} - '{target}'")
...
def add_defs(defs, *, namespace=None):
  ...
  if nspace is None:
    ...
    shorthand.logger.error(f"Definitions have already been added with namespace '{nspace}'")
  else:
...
def setup(sphinx):
  ...
  for new_role in ['link_to', 'link-to', 'shorthand-link-to', 'shorthand-link_to']:
