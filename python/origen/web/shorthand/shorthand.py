    ...
    for defs in app.config.shorthand_defs:
      if 'namespace' in defs:
        ...
        if 0 in shorthand_defs:
          logger.error("An un-namespaced definition group has already been defined. Please namespace any additional definitions!")
        else:
...
class Target:
  ...
  def generate_substitution(self, f):
    ...
    if self.is_substitution:
      ...
      if self.text:
        f.write(f'.. |{n}| replace:: :{self.target}:`{self.text} <>`\n')
      else:
        f.write(f'.. |{n}| replace:: :{self.target}:`{self.name} <>`\n')
    elif self.is_ref:
      if self.text:
        f.write(f'.. |{n}| replace:: :ref:`{self.text} <{self.target}>`\n')
      else:
        f.write(f'.. |{n}| replace:: :ref:`{self.name} <{self.target}>`\n')
      ...
      logger.error(f"Target '{n}' has unknown category '{self.category}'\n")
...
class ShorthandDefs:
  ...
  def __init__(self, app, opts):
    ...
    self.output_name = self.opts.get('output_name',
      self.namespace if self.namespace else (self.namespace or 'shorthand_defs')
    )
    self.output_dir = pathlib.Path(self.opts['output_dir']).resolve() if 'output_dir' in self.opts else None
      ...
      for name, t in targets.items():
        ...
        if isinstance(t, dict):
          ...
          if n in all_flattened:
            logger.warn(f"Clashing target: '{n}' has already been set to '{all_flattened[n]}'")
  ...
  def output_file(self):
    ...
    return (
      self.output_dir or 
      (shorthand_defs[0].output_dir if 0 in shorthand_defs else None) or
      default_output_dir or
      pathlib.Path(self.app.srcdir)
    ).joinpath(f"{self.output_name}.rst")
