...
class TestDefiningPins:
  def test_adding_pins_with_width(self, clean_falcon):
    origen.dut.add_pin("porta", width=4)
    assert origen.dut.pins.keys() == ["porta0", "porta1", "porta2", "porta3", "porta"]
    assert len(origen.dut.pins) == 5
    assert origen.dut.physical_pins.keys() == ["porta0", "porta1", "porta2", "porta3"]
    ...
    assert origen.dut.pin("porta").pin_names == ["porta0", "porta1", "porta2", "porta3"]
...
class TestGrouping:
  ...
  def test_collecting_using_ruby_like_syntax(self, clean_falcon, pins, ports):
  ...
  def test_exception_on_grouping_aliases_of_the_same_pin(self, clean_falcon, pins, grp):
    origen.dut.add_pin_alias("p1", "a1")
    with pytest.raises(OSError):
      origen.dut.group_pins("invalid", "p1", "p2", "a1", "p3")
    assert "invalid" not in origen.dut.pins

  def test_exception_on_grouping_duplicates_when_nested(self, clean_falcon, ports):
...
class TestCollecting:
  ...
  def test_exception_on_collecting_duplicates_when_nested_2(self, clean_falcon, ports):
